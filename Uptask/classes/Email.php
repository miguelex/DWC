...
class Email {
   ...
   protected $token;
    ...
    public function enviarInstrucciones() {
        ...
        $mail->setFrom('cuentas@uptask.com');
        $mail->addAddress('cuentas@uptask.com', 'uptask.com');
        $mail->Subject = 'Instrucciones para reestablecer contraseña';

        $mail->isHTML(true);
        $mail->CharSet = 'UTF-8';
        $contenido = "<html>";
        $contenido .= "<p><strong>Hola " . $this->nombre . "</strong>, has solicitado reestablecer tu contraseña. Solo debes seguir el siguiente enlace:</p>";
        $contenido .= "<p>Presiona aquí: <a href='http://localhost:8000/reestablecer?token=" . $this->token . "'>Reestablecer Contraseña</a></p>";
        $contenido .= "<p>Si tuviste problemas haciendo clic en el botón, copia y pega la siguiente URL en tu navegador: </p>";
        $contenido .= "<p>http://localhost:8000/reestablecer?token=" . $this->token . "</p>";
        $contenido .= "</html>";
        $mail->Body = $contenido;
